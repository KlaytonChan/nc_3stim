...
async function experimentInit() {
  ...
  preleft_2 = new visual.ButtonStim({
    win: psychoJS.window,
    name: 'preleft_2',
    text: '',
    font: 'Noto Sans TC',
  ...
  preleft_2.clock = new util.Clock();
  
  preright_2 = new visual.ButtonStim({
    win: psychoJS.window,
    name: 'preright_2',
    text: '',
    font: 'Noto Sans TC',
  ...
  preright_2.clock = new util.Clock();
  
  precenter_2 = new visual.TextBox({
    win: psychoJS.window,
    name: 'precenter_2',
    text: '',
    placeholder: 'Type here...',
...
function testRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    preleft_2.setText(Ltext);
    // reset preleft_2 to account for continued clicks & clear times on/off
    preleft_2.reset()
    preright_2.setText(Rtext);
    // reset preright_2 to account for continued clicks & clear times on/off
    preright_2.reset()
    precenter_2.setText(Ctext);
    psychoJS.experiment.addData('test.started', globalClock.getTime());
    testMaxDuration = null
    // keep track of which components have finished
    testComponents = [];
    testComponents.push(preleft_2);
    testComponents.push(preright_2);
    testComponents.push(precenter_2);
...
function testRoutineEachFrame() {
  return async function () {
    ...
    // *preleft_2* updates
    if (t >= 0 && preleft_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      preleft_2.tStart = t;  // (not accounting for frame time here)
      preleft_2.frameNStart = frameN;  // exact frame index
      
      preleft_2.setAutoDraw(true);
    }
    
    
    // if preleft_2 is active this frame...
    if (preleft_2.status === PsychoJS.Status.STARTED) {
    }
    
    if (preleft_2.status === PsychoJS.Status.STARTED) {
      // check whether preleft_2 has been pressed
      if (preleft_2.isClicked) {
        if (!preleft_2.wasClicked) {
          // store time of first click
          preleft_2.timesOn.push(preleft_2.clock.getTime());
          // store time clicked until
          preleft_2.timesOff.push(preleft_2.clock.getTime());
        } else {
          // update time clicked until;
          preleft_2.timesOff[preleft_2.timesOff.length - 1] = preleft_2.clock.getTime();
        }
        if (!preleft_2.wasClicked) {
          // end routine when preleft_2 is clicked
          continueRoutine = false;
          
        }
        // if preleft_2 is still clicked next frame, it is not a new click
        preleft_2.wasClicked = true;
      } else {
        // if preleft_2 is clicked next frame, it is a new click
        preleft_2.wasClicked = false;
      }
    } else {
      // keep clock at 0 if preleft_2 hasn't started / has finished
      preleft_2.clock.reset();
      // if preleft_2 is clicked next frame, it is a new click
      preleft_2.wasClicked = false;
    }
    
    // *preright_2* updates
    if (t >= 0 && preright_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      preright_2.tStart = t;  // (not accounting for frame time here)
      preright_2.frameNStart = frameN;  // exact frame index
      
      preright_2.setAutoDraw(true);
    }
    
    
    // if preright_2 is active this frame...
    if (preright_2.status === PsychoJS.Status.STARTED) {
    }
    
    if (preright_2.status === PsychoJS.Status.STARTED) {
      // check whether preright_2 has been pressed
      if (preright_2.isClicked) {
        if (!preright_2.wasClicked) {
          // store time of first click
          preright_2.timesOn.push(preright_2.clock.getTime());
          // store time clicked until
          preright_2.timesOff.push(preright_2.clock.getTime());
        } else {
          // update time clicked until;
          preright_2.timesOff[preright_2.timesOff.length - 1] = preright_2.clock.getTime();
        }
        if (!preright_2.wasClicked) {
          // end routine when preright_2 is clicked
          continueRoutine = false;
          
        }
        // if preright_2 is still clicked next frame, it is not a new click
        preright_2.wasClicked = true;
      } else {
        // if preright_2 is clicked next frame, it is a new click
        preright_2.wasClicked = false;
      }
    } else {
      // keep clock at 0 if preright_2 hasn't started / has finished
      preright_2.clock.reset();
      // if preright_2 is clicked next frame, it is a new click
      preright_2.wasClicked = false;
    }
    
    // *precenter_2* updates
    if (t >= 0.0 && precenter_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      precenter_2.tStart = t;  // (not accounting for frame time here)
      precenter_2.frameNStart = frameN;  // exact frame index
      
      precenter_2.setAutoDraw(true);
    }
    
    
    // if precenter_2 is active this frame...
    if (precenter_2.status === PsychoJS.Status.STARTED) {
    }
...
function testRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('preleft_2.numClicks', preleft_2.numClicks);
    psychoJS.experiment.addData('preleft_2.timesOn', preleft_2.timesOn);
    psychoJS.experiment.addData('preleft_2.timesOff', preleft_2.timesOff);
    psychoJS.experiment.addData('preright_2.numClicks', preright_2.numClicks);
    psychoJS.experiment.addData('preright_2.timesOn', preright_2.timesOn);
    psychoJS.experiment.addData('preright_2.timesOff', preright_2.timesOff);
